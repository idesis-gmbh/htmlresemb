...
// idesis GmbH
// Copy of /usr/local/share/fpcsrc/packages/fcl-xml/src/xmlwrite.pp
// Patched because of a line breaking problem.
// See method wrtIndent
...
begin
// idesis GmbH
// wrtChars deactivated because of a line breaking problem with added nodes.
// Without this patch span elements (for example) in headings would contain
// additional line breaks or whitespace which led to layout problems.
// (The patch was primarly needed for document which were created from more
// complex MultiMarkdown documents.)
...
begin
  ...
  begin
    ...
    // idesis GmbH
    // Patched because of "incorrect" behaviour:
    // For example, it raises an exception for the character '>'
    Sender.wrtChr(s[idx]);
end;
...
begin
  ...
  wrtChr('>');
  // idesis GmbH
  // Patched because of missing line break after DOCTYPE
  wrtStr(FLineBreak);
end;
